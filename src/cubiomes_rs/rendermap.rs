...
extern "C" {
    ...
    #[no_mangle]
    fn setupGeneratorMC113() -> LayerStack_0;
...
pub unsafe extern "C" fn parseOptions(
    ...
    if 0 == opts.seed {
        ...
        usage();
        exit(-1i32);
    } else if 0 == strlen(opts.ppmfn.as_mut_ptr()) {
        fprintf(
            stderr,
            b"Filename is required.\n\x00" as *const u8 as *const libc::c_char,
        );
        usage();
        exit(-1i32);
    } else {
...
unsafe extern "C" fn dist(mut spawn: Pos_0, mut x: libc::c_int, mut z: libc::c_int) -> libc::c_int {
...
#[no_mangle]
pub unsafe extern "C" fn findQuadRegions(mut seed: int64_t, mut radius: libc::c_int) -> Pos_0 {
    ...
    return Pos { x: 0i32, z: 0i32 };
}
#[no_mangle]
pub unsafe extern "C" fn findCenterFromRegion(mut quad: Pos_0) -> Pos_0 {
    return Pos {
...
#[no_mangle]
pub unsafe extern "C" fn findQuadCenter(mut seed: int64_t, mut radius: libc::c_int) -> Pos_0 {
    let mut quad: Pos_0 = findQuadRegions(seed, radius);
...
pub unsafe extern "C" fn writeMap(
    ...
    let mut center: Pos_0 = Pos { x: 0i32, z: 0i32 };
    let mut distfrom: Pos_0 = spawn;
        ...
        while row < rows {
            ...
            while i_0 < opts.width {
                ...
                if b < 256i32 {
                    ...
                    fprintf(
                        stderr,
                        b"INVALID BIOME!\x00" as *const u8 as *const libc::c_char,
                    );
                }
                i_0 += 1
            }
    ...
    fprintf(
        stderr,
        b"Distances to biomes:\n\x00" as *const u8 as *const libc::c_char,
    );
    let mut i_1: libc::c_int = 0i32;
    while i_1 < 256i32 {
        if distances[i_1 as usize] < 2147483647i32 {
            fprintf(
                stderr,
                b"    %23s: %5d\n\x00" as *const u8 as *const libc::c_char,
                biomeNames[i_1 as usize],
                distances[i_1 as usize],
            );
        }
        i_1 += 1
    }
...
pub unsafe extern "C" fn printCompositeCommand(
    ...
    let mut center: Pos_0 = Pos { x: 0i32, z: 0i32 };
    let mut huts: Pos_0 = findQuadRegions(opts.seed, 10240i32 / 16i32 / 32i32);
    let mut hutCenter: Pos_0 = findCenterFromRegion(huts);
    if 0 != opts.centerAtHuts {
        center = hutCenter
    }
    fprintf(
    ...
    let mut tl: Pos_0 = Pos { x: 0, z: 0 };
    let mut br: Pos_0 = Pos { x: 0, z: 0 };
    regionify(
        center,
        opts.width,
        opts.height,
    ...
    let mut strongholds: [Pos_0; 128] = [Pos { x: 0, z: 0 }; 128];
    findStrongholds(
        if 0 != opts.use_1_12 {
            MC_1_12 as libc::c_int
        } else {
...
unsafe fn main_0(mut argc: libc::c_int, mut argv: *mut *mut libc::c_char) -> libc::c_int {
    ...
    if fp.is_null() {
        ...
        if opts.oneBiome != -1i32 {
            g = setupFakeGenerator(opts.oneBiome)
        } else if 0 != opts.use_1_12 {
        ...
        // to a .png file.
        printCompositeCommand(opts, &mut g);
        return 0i32;
    };
}
pub fn main() -> () {

...
pub struct TypedArray<T>(Vec<T>);

impl<T> From<TypedArray<T>> for Vec<T> {
    fn from(x: TypedArray<T>) -> Self {
        x.0
    }
}
...
#[node_bindgen(name = "gen_test_seed_base_n_bits")]
...
//#[node_bindgen]
pub fn nbt_search(_zipped_world: TypedArray<u8>, _block_name: &str) -> Serde<Vec<NbtSearchResult>> {
    unimplemented!("nbt search not supported yet")
}

#[node_bindgen(name = "get_color_to_biome_map")]
pub fn get_color_to_biome_map() -> Value {
    let rgba_to_biome = biome_layers::color_to_biome_map();

    let h: HashMap<_, _> = rgba_to_biome
        .into_iter()
        .map(|(rgba, biome_id)| {
            let [r, g, b, _a] = rgba;
    ...
    Value::Object(h.into_iter().map(|(k, v)| (k, Value::from(v))).collect())
}

#[node_bindgen(name = "get_biome_id_to_biome_name_map")]
pub fn get_biome_id_to_biome_name_map() -> Value {
    let num_biomes = 256;
    ...
    Value::Object(h.into_iter().map(|(k, v)| (k, Value::String(v))).collect())
}

#[node_bindgen(name = "read_fragment_biome_map")]
pub async fn read_fragment_biome_map(
    zipped_world: JSArrayBuffer,
    _version: String,
    fx: i32,
    fy: i32,
    frag_size: u32,
    ...
    log::debug!("args: {:?}", (_version, fx, fy, frag_size, y_offset));
    log::debug!(
        "Called read_fragment_biome_map, zipped_world len: {}",
        zipped_world.len()
    );
    let zipped_world_vec = zipped_world.to_vec();
    log::debug!("zipped_world_vec len: {}", zipped_world_vec.len());
    use slime_seed_finder::anvil::ZipChunkProvider;
    use std::io::Cursor;
    // TODO: check if the input is actually a zipped_world, as it also may be a raw region file
    let mut zip_chunk_provider = ZipChunkProvider::new(Cursor::new(zipped_world.to_vec())).unwrap();

    log::debug!("zip chunk provider created");
    ...
    let v = biome_layers::draw_map_image(&map);

    log::debug!("Creating arraybuffer:");

    let vv = ArrayBuffer::new(v);

    log::debug!("Arraybuffer done: {:?}", vv);
